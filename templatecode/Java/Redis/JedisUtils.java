...
/**
 * Redis操作类   单机和sentinel集群可切换
 * jedis的版本是jedis-2.9.0.jar 
 */
public class Redis {
	...
	static final public Redis getInstance() {
		return instance;
	}
	...
	public void init(){
		boolean isSentinel = RedisConfig.isSentinel;
		if (!isSentinel) {
			initialPool();
		} else {
			initialSentinelPool();
	...
	private void initialPool() {
		int maxTotal = RedisConfig.maxTotal;
		int maxIdle = RedisConfig.maxIdle;
		int maxWaitMills = RedisConfig.maxWaitMillis;
		String host=  RedisConfig.ip;
		int port = RedisConfig.port;
		String password = RedisConfig.pass;
		boolean isSSL = RedisConfig.isSsl;
		...
		jedisPool = new JedisPool(config, host, port, maxWaitMills, password, isSSL);
	}
	...
	private void initialSentinelPool() {
		...
		int maxWaitMills = RedisConfig.maxWaitMillis;
		String password = RedisConfig.pass;
//		boolean isSSL = MicroConfig.readBoolean("redis.isSSL");
		String sentinelMasterName = RedisConfig.sentinelMasterName;
		String sentinelHosts = RedisConfig.ip;
		String[] sentinelHostsArray = sentinelHosts.split(",");
		Set<String> sentinelHostSet = new HashSet<String>();
		for (String host : sentinelHostsArray) {
			sentinelHostSet.add(host);
		}
		...
		config.setTestOnReturn(false);
		config.setTestWhileIdle(false);
		sentinelPool = new JedisSentinelPool(sentinelMasterName,sentinelHostSet , config, maxWaitMills,
				password);
	}
	...
	public void returnJedis(Jedis jedis) {
		jedis.close();
//		getRedisPool().returnResource(jedis);
	}
 
	public Set<String> scanForKeys(Jedis jedis, String patten) {
		...
		scanParams.match(patten);
		String cur = "";
 
		// 开始
		ScanResult<String> sr = jedis.scan(cur, scanParams);
		cur = sr.getStringCursor();
		all_keys.addAll(sr.getResult());
		while (!cur.isEmpty() && Integer.parseInt(cur) > 0) {
			sr = jedis.scan(cur, scanParams);
			cur = sr.getStringCursor();
			all_keys.addAll(sr.getResult());
		...
		if( elapse > 500 ) {
			logger.warn(String.format("Scan for %s used %d ms", patten, elapse));
		}
	...
	public class SortSet {
		...
		public Set<byte[]> zrange(byte[] key, int start, int end) {
			// ShardedJedis sjedis = shardedJedisPool.getResource();
			// Set<byte[]> set = sjedis.zrange(key, start, end);
			// shardedJedisPool.returnResource(sjedis);
			// return set;
			Jedis jedis = null;
			try {
